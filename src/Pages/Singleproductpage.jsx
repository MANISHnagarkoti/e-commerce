...
const Singleproductpage = () => {
  ...
  const changeindex = (i) => { pphotoindexnum(i) }
  ...
  return (
    ...
    <Wrapper>
      ...
      <Columnone>
        ...
        <Mainproductphoto>
          ...
          {/* {
            ...
            <img src={singleproductdata.image && singleproductdata.image[photoindexnum].url} alt="" />
          ...
          <ReactImageMagnify className='magnify-img'       {...{
            smallImage: {
              alt: "hola",
              isFluidWidth: true,
              src: singleproductdata.image && singleproductdata.image[photoindexnum].url
            },
            largeImage: {
              src: singleproductdata.image && singleproductdata.image[photoindexnum].url,
              width: 1300,
      ...
      <Columntwo>
        ...
        <div className='productprize'>Prize :    <FormatPrice price={singleproductdata.price} /> </div>
        <div className='productcom'>Company : {singleproductdata.company}</div>
        <div className='productdes'> {singleproductdata.description}  </div>
        <div style={singleproductdata.stock > 0 ? { color: "green" } : { color: "red" }} className="productstock" >{singleproductdata.stock > 0 ? "Stock availiable" : "not availaible"}</div>
        <Counter margintop={"25px"} state={currentstock} add={add} sub={sub} />


        <Colorpicker cchangecolor={cchangecolor} data={singleproductdata.colors} />

        <Allphotocontainer>
          {
            singleproductdata.image && singleproductdata.image.map((e, i) => {

              return (

                <img key={i} style={{ cursor: "pointer" }} onMouseOver={() => { changeindex(i) }} src={e.url} alt="aa" />
        ...
        <Addtocartbutton add={() => { dispatch(addtocart({ singleproductdata, selectedcolor, currentstock })) }} />
